...
public class AnalyticsController {
    ...
    public ResponseEntity<Map<String, Object>> getPerformanceMetrics() {
        Map<String, Object> performanceMetrics = Map.of(
            ...
            "errorMetrics", Map.of(
                "errorRate", calculateErrorRate(),
                "totalErrors", businessMetrics.getErrorCount()
            )
        );
    ...
    public ResponseEntity<Map<String, Object>> getBusinessMetricsSummary() {
        Map<String, Object> businessSummary = Map.of(
            "userMetrics", Map.of(
                "totalRegistrations", businessMetrics.getUserRegistrationCount(),
                "totalLogins", businessMetrics.getUserLoginCount(),
                "activeUsers", businessMetrics.getActiveUsers()
            ),
            "assignmentMetrics", Map.of(
                "totalCreated", businessMetrics.getAssignmentCreatedCount(),
                "totalSubmitted", businessMetrics.getAssignmentSubmittedCount(),
                "totalGraded", businessMetrics.getAssignmentGradedCount(),
                "pending", businessMetrics.getPendingAssignments(),
                "averageGradingTime", businessMetrics.getAverageGradingTime()
            ),
            "submissionMetrics", Map.of(
                "totalUploaded", businessMetrics.getSubmissionUploadCount(),
                "plagiarismChecks", businessMetrics.getSubmissionPlagiarismCheckCount(),
                "totalSubmissions", businessMetrics.getTotalSubmissions(),
                "averagePlagiarismTime", businessMetrics.getAveragePlagiarismCheckTime()
            ),
            "courseMetrics", Map.of(
                "totalCreated", businessMetrics.getCourseCreatedCount(),
                "totalEnrollments", businessMetrics.getTotalEnrollments(),
                "activeCourses", businessMetrics.getActiveCourses()
            )
        );
    ...
    private double calculateCacheHitRate() {
        double hits = businessMetrics.getCacheHitCount();
        double misses = businessMetrics.getCacheMissCount();
        return (hits + misses) > 0 ? (hits / (hits + misses)) * 100 : 0;
    }

    private double calculateErrorRate() {
        double errors = businessMetrics.getErrorCount();
        // Note: We don't track total requests separately, so use a reasonable approximation
        return errors > 0 ? (errors / (errors + 100)) * 100 : 0; // Simple approximation
    }

    private double calculateAverageResponseTime() {
        return businessMetrics.getAverageApiResponseTime();
    }
    ...
    private double calculateDatabaseQueryRate() {
        return businessMetrics.getDbQueryCount() / 60.0; // queries per minute
    }

    private double calculateSlowQueryRate() {
        return businessMetrics.getSlowDbQueryCount() / 60.0; // slow queries per minute
    }

    private double calculateAverageQueryTime() {
        return businessMetrics.getAverageDbQueryTime();
    }

    private double calculateAverageCacheTime() {
        return businessMetrics.getAverageCacheOperationTime();
    }
}
